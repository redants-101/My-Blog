...
var Xclass = function(config){
  ...
  if(typeof(config.reset)!="undefined" && config.reset!==null && config.reset===false){
    for(var key in config){
      if(config[key]!=null){
        cstr += config[key].toString();
      }
    }
    that.cstr = that.cstrRep(cstr);
  }else{
    that.cstr = cstr;
  }
...
Xclass.pt.findxoff = function(){
  ...
  for(var i=0;i<xoff.length;i++){
    cstr = that.dataset(xoff[i],'cstr');
    if(cstr && cstr==that.cstr){
      xoff[i].style.zIndex = c.zindex;
      setTimeout(function(){
        xoff[i].classList.remove('xoff');
      },1);
      return true;
    }
  }
...
Xclass.pt.attr = function(){
  ...
  //弹窗层
    ...
    if(c.model=='open'){
      that.dataset(xtipdiv,'cstr',that.cstr);
    }
    if(c.shade===true){
...
//参数处理
Xclass.pt.cstrRep = function(str){
  str = str.replace(/[\s\n\r]/g,""); //空格换行回车
  str = str.replace(/true/g,"1_");
  str = str.replace(/false/g,"0_");
    ...
    if(c.reset===false){
      ...
      setTimeout(function(){
        ...
        if(c.model=='open' && c.type=='photo'){
          if(that.ifmob===true){
            var con = xtipdiv.getElementsByClassName('xtiper_content')[0];
            con.style.backgroundColor = 'rgba(0,0,0,1)';
          }
          var li = xtipdiv.getElementsByClassName('xtiper_photo_li');
          if(li.length>0){
            for(var i=0;i<li.length;i++){
...
window.xtip = {
  ...
  open: function(config){
    ...
    o.reset = config.reset!=null ? config.reset : false;
    o.zindex = config.zindex ? config.zindex : 99999;
